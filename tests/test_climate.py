...
from aioesphomeserver import ClimateEntity, Device
...
class ExampleClimateEntity(ClimateEntity):
    def __init__(self, device, name, object_id, **kwargs):
        ...
        self.running = True
        logger.info(
            f"Initialized TestClimateEntity with name {self.name}, object_id {self.object_id}"
        )
        ...
        while self.running:
            try:
                ...
                if random.choice([True, False]):
                    changes["target_temperature"] = round(
                        random.uniform(
                            self.visual_min_temperature, self.visual_max_temperature
                        ),
                        1,
                    )
                if random.choice([True, False]):
                ...
                if self.supports_current_temperature and random.choice([True, False]):
                    changes["current_temperature"] = round(
                        random.uniform(
                            self.visual_min_temperature, self.visual_max_temperature
                        ),
                        1,
                ...
                if self.supports_target_humidity and random.choice([True, False]):
                    changes["target_humidity"] = round(
                        random.uniform(
                            self.visual_min_humidity, self.visual_max_humidity
                        ),
                        1,
                    )
                if self.supports_current_humidity and random.choice([True, False]):
                    changes["current_humidity"] = round(
                        random.uniform(
                            self.visual_min_humidity, self.visual_max_humidity
                        ),
                        1,
                    )
                if self.supports_two_point_target_temperature and random.choice(
                    [True, False]
                ):
                    changes["target_temperature_low"] = round(
                        random.uniform(
                            self.visual_min_temperature, self.visual_max_temperature
                        ),
                        1,
                    )
                    changes["target_temperature_high"] = round(
                        random.uniform(
                            changes["target_temperature_low"],
                ...
                if changes:
                    log_changes = {
                        k: self.value_to_string(k, v) for k, v in changes.items()
                    }
...
async def main():
    ...
    await asyncio.gather(
        device.run(api_port=6053, web_port=8081), climate_entity.random_values()
    )
